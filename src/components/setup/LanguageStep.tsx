    ...
    <Card>
      ...
      <CardContent>
        <RadioGroup value={value} onValueChange={(v) => onChange(v as "en" | "ta")} className="space-y-4">
          <div className="flex items-center gap-4 rounded-lg border p-4 hover:border-primary transition-colors cursor-pointer" onClick={() => onChange("en")}>
            <RadioGroupItem value="en" id="lang-en" />
            <Label htmlFor="lang-en" className="text-lg font-medium cursor-pointer">{t.languageEn}</Label>
          </div>
          <div className="flex items-center gap-4 rounded-lg border p-4 hover:border-primary transition-colors cursor-pointer" onClick={() => onChange("ta")}>
